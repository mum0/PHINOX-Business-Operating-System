...
/**
 * ضمان وجود الأعمدة الكاملة
 */
function ensureSuppliersColumns(){
  const sheet = getSheet(APP.SHEETS.SUPPLIERS);
  const required = [
    "Supplier ID", "Company", "Contact", "Phone", "Email", "Material", "Lead Time", "Rating",
    "Payment Terms", "Address", "Status", "Total Orders", "Total Spent", "Avg Delivery Days", "Notes", "Created At", "Updated At"
  ];
...
function addSupplier(supplier){
  ...
  const row = [
    ...
    supplier.email || "",
    supplier.material || "",
    toNumber(supplier.leadTime),
    clamp(toNumber(supplier.rating), 0, 5),
  ...
  sheet.appendRow(row);
  logActivity(getCurrentMember(), "إضافة مورد", APP.SHEETS.SUPPLIERS, row[SUP_COL.SUPPLIER_ID], "", row[SUP_COL.COMPANY]);
...
function getSupplier(supplierId){
  ...
  for(const s of suppliers){
    if(s[SUP_COL.SUPPLIER_ID] === supplierId){
      return s;
    }
  }
  return null;
...
/**
 * مورد بواسطة اسم الشركة
 */
function getSupplierByCompany(company){
  const suppliers = getSuppliers();
  for(const s of suppliers){
    if(s[SUP_COL.COMPANY] === company){
      return s;
    }
  }
  return null;
...
function updateSupplier(supplierId, updates){
  ...
  for(let i = 1; i < data.length; i++){
    if(data[i][SUP_COL.SUPPLIER_ID] === supplierId){
      ...
      const map = {
        company: SUP_COL.COMPANY,
        contact: SUP_COL.CONTACT,
        phone: SUP_COL.PHONE,
        email: SUP_COL.EMAIL,
        material: SUP_COL.MATERIAL,
        leadTime: SUP_COL.LEAD_TIME,
        rating: SUP_COL.RATING,
        paymentTerms: SUP_COL.PAYMENT_TERMS,
        address: SUP_COL.ADDRESS,
        status: SUP_COL.STATUS,
        notes: SUP_COL.NOTES
      };
      ...
      if(updates.rating !== undefined){
        row[SUP_COL.RATING] = clamp(toNumber(updates.rating), 0, 5);
      }
      row[SUP_COL.UPDATED_AT] = now();
      sheet.getRange(i + 1, 1, 1, row.length).setValues([row]);
      logActivity(getCurrentMember(), "تحديث مورد", APP.SHEETS.SUPPLIERS, supplierId, oldCompany, row[SUP_COL.COMPANY]);
      return true;
    }
  }
  return false;
...
function deleteSupplier(supplierId){
  ...
  for(let i = 1; i < data.length; i++){
    if(data[i][SUP_COL.SUPPLIER_ID] === supplierId){
      sheet.deleteRow(i + 1);
      logActivity(getCurrentMember(), "حذف مورد", APP.SHEETS.SUPPLIERS, supplierId, "", "");
      return true;
    }
  }
  return false;
...
function activateSupplier(supplierId){
  return updateSupplier(supplierId, {status: SUPPLIER_STATUS.ACTIVE});
}
...
function deactivateSupplier(supplierId){
  return updateSupplier(supplierId, {status: SUPPLIER_STATUS.INACTIVE});
}

/**
 * حظر مورد
 */
function blockSupplier(supplierId){
  return updateSupplier(supplierId, {status: SUPPLIER_STATUS.BLOCKED});
}
...
function activeSuppliers(){
  return getSuppliers().filter(s => s[SUP_COL.STATUS] === SUPPLIER_STATUS.ACTIVE);
}
...
function averageSupplierRating(){
  ...
  let total = 0;
  suppliers.forEach(s => {
    total += toNumber(s[SUP_COL.RATING]);
  });
...
function averageLeadTime(){
  ...
  let total = 0;
  suppliers.forEach(s => {
    total += toNumber(s[SUP_COL.LEAD_TIME]);
  });
...
function logPurchase(supplierId, purchase){
  ...
  const row = [
    ...
    purchase.status || PURCHASE_STATUS.ORDERED,
    purchase.notes || ""
  ];
  sheet.appendRow(row);
...
function updatePurchaseStatus(purchaseId, status, deliveryDate){
  ...
  for(let i = 1; i < data.length; i++){
    if(data[i][PUR_COL.PURCHASE_ID] === purchaseId){
      ...
      if(deliveryDate){
        row[PUR_COL.DELIVERY_DATE] = deliveryDate;
      }
      sheet.getRange(i + 1, 1, 1, row.length).setValues([row]);
...
function getSupplierTotalSpent(supplierId){
  ...
  let total = 0;
  purchases.forEach(p => {
    if(p[PUR_COL.STATUS] !== PURCHASE_STATUS.CANCELLED){
      total += toNumber(p[PUR_COL.TOTAL_COST]);
    }
  });
...
function getSupplierOrderCount(supplierId){
  return getSupplierPurchases(supplierId).filter(p => 
    p[PUR_COL.STATUS] !== PURCHASE_STATUS.CANCELLED
  ).length;
}

/**
 * متوسط أيام التسليم الفعلية
 */
...
function getSupplierKPIs(supplierId){
  ...
  return {
    ...
    company: supplier[SUP_COL.COMPANY],
    totalOrders: total,
    deliveryRate: deliveryRate,
    onTimeRate: onTimeRate,
    returnRate: returnRate,
    cancelRate: cancelRate,
    avgDeliveryDays: getSupplierActualDeliveryDays(supplierId),
    leadTime: toNumber(supplier[SUP_COL.LEAD_TIME]),
    totalSpent: getSupplierTotalSpent(supplierId),
...
/**
 * ترتيب الموردين حسب الموثوقية
 */
function getSupplierRanking(){
  const suppliers = activeSuppliers();
  const ranked = [];
  suppliers.forEach(s => {
    ranked.push(getSupplierKPIs(s[SUP_COL.SUPPLIER_ID]));
  });
 ...
 * أفضل الموردين
 */
function topSuppliers(limit){
  return getSupplierRanking().slice(0, limit || 5);
}

/**
 * الموردون الأكثر إنفاقاً
 */
function topSuppliersBySpend(limit){
  ...
  suppliers.forEach(s => {
    list.push({
      supplierId: s[SUP_COL.SUPPLIER_ID],
      company: s[SUP_COL.COMPANY],
...
function getSuppliersSummary(){
  return {
    ...
    averageRating: averageSupplierRating(),
...
/**
 * بناء لوحة تحكم الموردين
 */
function buildSuppliersDashboard(){
  const dashboard = getSheet(APP.SHEETS.DASHBOARD);
  const summary = getSuppliersSummary();
  dashboard.appendRow([]);
  dashboard.appendRow(["ملخص الموردين"]);
  dashboard.appendRow(["إجمالي الموردين", summary.totalSuppliers]);
  dashboard.appendRow(["النشطون", summary.activeSuppliers]);
  dashboard.appendRow(["متوسط التقييم", summary.averageRating]);
  dashboard.appendRow(["متوسط وقت التوريد", summary.averageLeadTime]);
  dashboard.appendRow(["إجمالي المشتريات", summary.totalPurchaseValue]);
  const top = topSuppliers(3);
  if(top.length > 0){
    dashboard.appendRow([]);
    dashboard.appendRow(["أفضل الموردين"]);
    top.forEach(s => {
      dashboard.appendRow([s.company, s.reliabilityScore]);
    });
  }
}

/**
 * تحديث كامل لوحة الموردين
 */
function refreshSuppliers(){
  ensureSuppliersColumns();
  const suppliers = getSuppliers();
  suppliers.forEach(s => {
    recalculateSupplierStats(s[SUP_COL.SUPPLIER_ID]);
  });
