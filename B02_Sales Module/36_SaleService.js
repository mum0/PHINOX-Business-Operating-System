 ...
 * UPDATED: Phase 8A — date-range queries for revenue/COGS/profit.
 */
...
const SaleService = (function() {
 ...
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 where: function(sale) {
 var created = new Date(sale.createdAt);
 ...
 var result = getSalesByDateRange(startDate, endDate);
 var sales = result && result.data ? result.data : [];
 return _round(sales.reduce(function(acc, s) { return acc + _toNumber(s.totalAmount); }, 0), 2);
 }

 function getTotalCOGSByDateRange(startDate, endDate) {
 var result = getSalesByDateRange(startDate, endDate);
 var sales = result && result.data ? result.data : [];
 return _round(sales.reduce(function(acc, s) { return acc + _toNumber(s.cogs); }, 0), 2);
 }

 function getGrossProfitByDateRange(startDate, endDate) {
 return _round(getTotalRevenueByDateRange(startDate, endDate) - getTotalCOGSByDateRange(startDate, endDate), 2);
 }
 ...
 getTotalRevenueByDateRange: getTotalRevenueByDateRange,
 getTotalCOGSByDateRange: getTotalCOGSByDateRange,
 getGrossProfitByDateRange: getGrossProfitByDateRange
 };
})();