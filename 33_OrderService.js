 ...
 * UPDATED: Phase 0 — date-range queries + aliases.
 * UPDATED: Phase 8A — customer APIs.
 */
...
const OrderService = (function() {
 ...
 function totalOrdersByDateRange(startDate, endDate) {
 var result = getOrdersByDateRange(startDate, endDate);
 return result && result.data ? result.data.length : 0;
 }

 function getOrdersByStatusAndDateRange(status, startDate, endDate) {
 var s = startDate ? new Date(startDate) : null;
 var e = endDate ? new Date(endDate) : null;
 return OrderRepository.findAll({
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 ...
 function getTotalOrderAmountByDateRange(startDate, endDate) {
 var result = getOrdersByDateRange(startDate, endDate);
 var orders = result && result.data ? result.data : [];
 return _round(orders.reduce(function(acc, o) { return acc + _toNumber(o.totalAmount); }, 0), 2);
 }

 // ============ PHASE 0: BACKWARD COMPATIBILITY ALIASES ============

 function getOrderById(id) { return getOrder(id); }
 function getTotalOrderAmount() { return getTotalSales(); }

 // ============ PHASE 8A: CUSTOMER APIs ============

 function getCustomers() {
 var result = OrderRepository.findAll({ limit: CONFIG.PAGINATION.MAX_LIMIT });
 var emails = {};
 result.data.forEach(function(o) {
 if (o.customerEmail) emails[o.customerEmail] = true;
 });
 return Object.keys(emails).sort();
 }

 function getCustomerStats(email) {
 var result = getOrdersByCustomer(email);
 var orders = result && result.data ? result.data : [];
 if (orders.length === 0) return null;
 orders.sort(function(a, b) { return new Date(a.createdAt) - new Date(b.createdAt); });
 var firstOrder = orders[0];
 var lastOrder = orders[orders.length - 1];
 var totalAmount = orders.reduce(function(acc, o) { return acc + _toNumber(o.totalAmount); }, 0);
 return {
 email: email,
 ...
 function getCustomerTotalAmount(email) {
 var result = getOrdersByCustomer(email);
 var orders = result && result.data ? result.data : [];
 return _round(orders.reduce(function(acc, o) { return acc + _toNumber(o.totalAmount); }, 0), 2);
 }
 ...
 getOrdersByDateRange: getOrdersByDateRange, totalOrdersByDateRange: totalOrdersByDateRange,
 getOrdersByStatusAndDateRange: getOrdersByStatusAndDateRange, getTotalOrderAmountByDateRange: getTotalOrderAmountByDateRange,
 getTotalSales: getTotalSales, totalOrders: totalOrders,
 getOrderById: getOrderById, getTotalOrderAmount: getTotalOrderAmount,
 getCustomers: getCustomers, getCustomerStats: getCustomerStats,
 getCustomerOrderCount: getCustomerOrderCount, getCustomerTotalAmount: getCustomerTotalAmount
 }
