...
const InventoryService = (function() {
    ...
    function _validateInput(data, isUpdate) {
      ...
      fields.forEach(function(f) {
        if (InventorySchema.VALIDATION[f]) schema[f] = InventorySchema.VALIDATION[f];
      });
      if (!isUpdate) {
    ...
    function _checkStockInvariant(item) {
      const q = _toNumber(item.quantity);
      const r = _toNumber(item.reserved);
    ...
    function createItem(data) {
      ...
      _recalculateAvailable(item);
      _autoStatus(item);
      _checkStockInvariant(item);
    ...
    function updateItem(id, updates) {
      ...
      _recalculateAvailable(merged);
      _autoStatus(merged);
      _checkStockInvariant(merged);
      data.available = merged.available;
      data.status = merged.status;
      const updated = InventoryRepository.update(id, data);
      Logger.info('InventoryService', 'Item updated', { id: id, sku: updated.sku });
      return updated;
    }
    ...
    function reserveStock(sku, qty) {
      ...
      const updated = updateItem(item.id, { reserved: newReserved });
    ...
    function commitStock(sku, qty) {
      ...
      const updated = updateItem(item.id, { quantity: newQuantity, reserved: newReserved });
    ...
    function restock(sku, qty) {
      ...
      if (!item) throw ErrorHandler.notFound('SKU', sku, 'InventoryService');
      const newQuantity = _toNumber(item.quantity) + qty;
      const updated = updateItem(item.id, { quantity: newQuantity });
    ...
    function getItemsByStatus(status) {
      return InventoryRepository.findAll({
        limit: CONFIG.PAGINATION.MAX_LIMIT,
        where: function(item) { return item.status === status; }
      });
    }
    ...
    function getInventoryValue() {
      ...
      return _round(result.data.reduce(function(acc, item) {
        return acc + (_toNumber(item.quantity) * _toNumber(item.cost));
      }, 0), 2);
    }
    ...
    function getInventoryRetailValue() {
      const result = InventoryRepository.findAll({ limit: CONFIG.PAGINATION.MAX_LIMIT });
      return _round(result.data.reduce(function(acc, item) {
        return acc + (_toNumber(item.available) * _toNumber(item.price));
      }, 0), 2);
    }
    ...
    return {
      ...
      getInventoryRetailValue: getInventoryRetailValue,
