 ...
 * UPDATED: Phase 8A — completedAt/approvedAt auto-tracking + date-range queries
 */
...
const TaskService = (function() {
 ...
 // Phase 8A: Auto-track completedAt/approvedAt on status transitions
 if (data.status !== undefined && data.status !== existing.status) {
 _validateStatusTransition(existing.status, data.status);
 if (data.status === S.APPROVED) {
 data.completion = 100;
 data.approvedAt = _now().toISOString();
 data.completedAt = _now().toISOString();
 }
 if (data.status === S.REJECTED || data.status === S.CANCELLED) {
 data.completedAt = _now().toISOString();
 }
 }
 ...
 function getTasksByStatusAndDateRange(status, startDate, endDate) {
 var s = startDate ? new Date(startDate) : null;
 var e = endDate ? new Date(endDate) : null;
 return TaskRepository.findAll({
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 ...
 function getCompletedTasksByDateRange(startDate, endDate) {
 var s = startDate ? new Date(startDate) : null;
 var e = endDate ? new Date(endDate) : null;
 return TaskRepository.findAll({
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 ...
 if (s && completed < s) return false;
 if (e && completed > e) return false;
 return true;
 }
 });
 }

 function getAverageCompletionTime(startDate, endDate) {
 var result = getCompletedTasksByDateRange(startDate, endDate);
 var tasks = result && result.data ? result.data : [];
 if (tasks.length === 0) return 0;
 var totalDays = 0, count = 0;
 tasks.forEach(function(t) {
 var created = Utils.safeDate(t.createdAt);
 var completed = Utils.safeDate(t.completedAt);
 if (created && completed) {
 totalDays += _daysBetween(created, completed);
 count++;
 }
 });
 return count > 0 ? _round(totalDays / count, 2) : 0;
 }

 function getOnTimeRate(startDate, endDate) {
 var result = getCompletedTasksByDateRange(startDate, endDate);
 var tasks = result && result.data ? result.data : [];
 if (tasks.length === 0) return 0;
 var onTime = 0;
 tasks.forEach(function(t) {
 var completed = Utils.safeDate(t.completedAt);
 var due = Utils.safeDate(t.dueDate);
 if (completed && due && completed <= due) onTime++;
 });
 return _round((onTime / tasks.length) * 100, 2);
 }

 function getOverdueTasks(startDate, endDate) {
 var s = startDate ? new Date(startDate) : null;
 var e = endDate ? new Date(endDate) : null;
 return TaskRepository.findAll({
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 where: function(t) {
 if ([S.APPROVED, S.REJECTED, S.CANCELLED].indexOf(t.status) > -1) return false;
 var due = Utils.safeDate(t.dueDate);
 if (!due) return false;
 var today = _now();
 ...
 function getTasksByAssigneeAndDateRange(assignee, startDate, endDate) {
 var s = startDate ? new Date(startDate) : null;
 var e = endDate ? new Date(endDate) : null;
 return TaskRepository.findAll({
 limit: CONFIG.PAGINATION.MAX_LIMIT,
 where: function(t) {
 if (t.assignedTo !== assignee) return false;
 var created = new Date(t.createdAt);
 if (s && created < s) return false;
 if (e && created > e) return false;
 return true;
 ...
 function getAverageQuality(startDate, endDate) {
 var result = getTasksByDateRange(startDate, endDate);
 var tasks = result && result.data ? result.data : [];
 if (tasks.length === 0) return 0;
 return _round(tasks.reduce(function(a, t) { return a + _toNumber(t.quality); }, 0) / tasks.length, 2);
 }
 ...
 getTasksByDateRange: getTasksByDateRange, getTasksByStatusAndDateRange: getTasksByStatusAndDateRange,
 getCompletedTasksByDateRange: getCompletedTasksByDateRange, getAverageCompletionTime: getAverageCompletionTime,
 getOnTimeRate: getOnTimeRate, getOverdueTasks: getOverdueTasks,
 getTasksByAssigneeAndDateRange: getTasksByAssigneeAndDateRange, getAverageQuality: getAverageQuality,
